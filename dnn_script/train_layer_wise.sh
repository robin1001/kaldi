  ...
  case "$network_type" in
    dnn)
      utils/nnet/make_nnet_proto.py $proto_opts \
        ...
        $num_fea $num_tgt 1 $hid_dim >$nnet_proto || exit 1 
	  #robin1001 prepare dnn hidden layers
   	  cat > $dir/hidden.conf <<EOF
<NnetProto>
<AffineTransform> <InputDim> $hid_dim <OutputDim> $hid_dim <BiasMean> -2.000000 <BiasRange> 4.000000 <ParamStddev> 0.043805
...
EOF
    ...
    lstm)
	  ...
	  #robin1001, lstm adding layers 
	  input_dim=`sed -n '2p' $nnet_proto | awk '{print $5}'`
	  cell_dim=`sed -n '2p'  $nnet_proto | awk '{print $7}'`
   	  cat > $dir/hidden.conf <<EOF
<NnetProto>
<LstmProjectedStreams> <InputDim> $input_dim <OutputDim> 512 <CellDim> $cell_dim <ParamScale> 0.010000 <ClipGradient> 5.000000
</NnetProto>
EOF
      ;;
    lstm-dnn)
	  ...
	  input_dim=`sed -n '2p' $nnet_proto | awk '{print $5}'`
   	  cat > $dir/hidden.conf <<EOF
<NnetProto>
<AffineTransform> <InputDim> $input_dim <OutputDim> $input_dim <BiasMean> -2.000000 <BiasRange> 4.000000 <ParamStddev> 0.043805
<Sigmoid> <InputDim> $input_dim <OutputDim> $input_dim
</NnetProto>
EOF
...
dnn_script/train_scheduler_layer_wise.sh \
  --feature-transform $feature_transform \
  --learn-rate $learn_rate \
  --randomizer-seed $seed \
  --num-hidden-layers $hid_layers \
  ${train_opts} \
